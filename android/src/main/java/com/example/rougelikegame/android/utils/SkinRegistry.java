package com.example.rougelikegame.android.utils;

import com.example.rougelikegame.android.models.meta.Skin;

import java.util.ArrayList;
import java.util.List;

public class SkinRegistry {

    private static final List<Skin> SKINS = createSkins();

    private static List<Skin> createSkins() {
        List<Skin> skins = new ArrayList<>();

        // Default
        skins.add(new Skin(
            "default", "Default",
            0, Skin.UnlockType.DEFAULT,
            "skins/player_default.png"
        ));

        // Shop skins
        skins.add(new Skin(
            "red", "Red",
            5, Skin.UnlockType.SHOP,
            "skins/player_red.png"
        ));

        skins.add(new Skin(
            "red_knight", "Red Knight",
            50, Skin.UnlockType.SHOP,
            "skins/red_knight.png"
        ));

        skins.add(new Skin(
            "shadow", "Shadow",
            100, Skin.UnlockType.SHOP,
            "skins/player_shadow.png"
        ));

        skins.add(new Skin(
            "angel", "Angel",
            1000, Skin.UnlockType.SHOP,
            "skins/player_angel.png"
        ));

        // Achievement skins
        skins.add(new Skin(
            "skin_wave_5", "Ruby Knight",
            Skin.UnlockType.ACHIEVEMENT,
            "wave_5",
            "skins/wave_5.png"
        ));

        skins.add(new Skin(
            "skin_wave_10", "Ice Knight",
            Skin.UnlockType.ACHIEVEMENT,
            "wave_10",
            "skins/wave_10.png"
        ));

        skins.add(new Skin(
            "skin_100_kills", "Slayer",
            Skin.UnlockType.ACHIEVEMENT,
            "kills_100",
            "skins/100_kills.png"
        ));

        skins.add(new Skin(
            "skin_25_coins", "Collector",
            Skin.UnlockType.ACHIEVEMENT,
            "coins_25",
            "skins/25_coins.png"
        ));

        skins.add(new Skin(
            "skin_first_win", "Champion",
            Skin.UnlockType.ACHIEVEMENT,
            "first_win",
            "skins/first_win.png"
        ));

        return skins;
    }

    public static List<Skin> getAllSkins() {
        return SKINS;
    }

    public static Skin getSkinById(String id) {
        if (id == null) return getDefaultSkin();

        for (Skin skin : SKINS) {
            if (skin.getId().equals(id)) {
                return skin;
            }
        }
        return getDefaultSkin();
    }

    public static Skin getDefaultSkin() {
        return SKINS.get(0);
    }
}
